...
class Spider_Initalizer():
    def __init__(self, db_name:str) -> None:
        self.db_id = db_name
        schema_path = Path(db_root_path).parent / "tables.json"
        with open(schema_path) as f:
            self.table_json = json.load(f) # weird format, but it's a list of one element
        for single_table_info in self.table_json:
            if single_table_info['db_id'] == self.db_id:
                self.table_json = single_table_info
                break
